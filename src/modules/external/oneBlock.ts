...
export class OneBlock extends Manager {
  ...
  async _init(): Promise<void> {
    ...
    this._blockList = BlockTypes.getAll().filter(
      (b) => !b.id.startsWith("minecraft:element_") && !this._banned.some((bn) => b.id.includes(bn))
    );
  ...
  _main(): void {
    ...
    if (!block || (!block.isAir && !block.isLiquid && !block.isWaterlogged)) return;
    ...
    if (!hasNearby) this._blockList = this._blockList.filter((b) => !this._noColl.some((nc) => b.id.includes(nc)));

    block.setType(this._blockList[Math.floor(Math.random() * this._blockList.length)].id);
